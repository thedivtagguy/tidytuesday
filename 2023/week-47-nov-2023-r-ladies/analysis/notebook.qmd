...
%>%
  mutate(similar = lapply(similar, function(x) {
    df <- tibble::as_tibble(t(x))
    if (ncol(df) > 0) {
      pivot_longer(df,
                   everything(),
                   names_to = "id",
      ...
      tibble(id = character(), value = numeric()) 
    }
  }))

jsonlite::write_json(output, "website/src/routes/meetup-data.json")
```
