...
top_countries <- rankings_cumulative %>%
  ...
  ungroup() %>%
  slice_max(order_by = cumulative_medals, n = 4) %>%
  pull(country)
...
overall <- ggplot(rankings_cumulative,
  ...
  geom_line(aes(
    ...
    alpha = ifelse(country %in% top_countries, 1, 0.2)
  ), size = 1.2) +
  geom_point(
 ...
 nudge_y = 1.5,
  ...
  theme_minimal() +
  labs(y = "", x = "",  title = str_wrap("Total Medals in International Math Olympiad by Country", 38),
  subtitle = str_wrap("Nearly 115 countries now send their top students to the IMO each year. USA leads the medal table with 289 medals, just ahead of Romania, while countries like China have risen quickly despite later starts.", 52)) +
...
((free(p1, side = "t") / p2 + plot_layout(axes = "collect")) | free(overall) | (free(p3, side = "t") / p4 + plot_layout(axes = "collect")) )
