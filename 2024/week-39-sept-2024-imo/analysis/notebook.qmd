...
rankings <- country %>%
  mutate(
    total_score = rowSums(select(., p1:p6), na.rm = TRUE),
    perfect_scores = rowSums(select(., p1:p6) == 7, na.rm = TRUE),
    total_medals = awards_gold + awards_silver + awards_bronze
  ) %>%
  group_by(year) %>%
  mutate(rank = if_else(
    !is.na(p1) &
      !is.na(p2) & !is.na(p3) & !is.na(p4) & !is.na(p5) & !is.na(p6),
    dense_rank(-total_score),
    NA_integer_
  ))
...
top_countries <- medals_cumulative %>%
  group_by(country) %>%
...
overall_data <- medals_cumulative %>%
  group_by(country) %>%
  slice_max(year, n = 1) %>%
  ungroup() %>%
  slice_max(cumulative_medals, n = 1) %>%
  mutate(
    total_countries = n_distinct(medals_cumulative$country),
    top_country = country,
    top_medals = cumulative_medals,
    second_country = top_countries[2],
    second_medals = filter(medals_cumulative, country == second_country & year == max(year))$cumulative_medals
  ) %>%
  slice(1) %>%
  mutate(subtitle = str_wrap(glue::glue("Nearly {total_countries} countries now send their top students to the IMO each year. {top_country} leads the medal table with {top_medals} medals, just ahead of {second_country}, while China has risen quickly despite a later start."), 60))

overall <- ggplot(medals_cumulative,
  ...
  labels = function(x) {
    ifelse(x %% 10 == 0, as.character(x), substr(as.character(x), 3, 4))
  }
) +
      theme(
      ...
      theme_ipsum_pub(grid = "y", axis_text_size = 16, plot_title_size = 28, subtitle_size = 18) + 
...
trends <- rankings %>%
  group_by(country) %>%
  #drop_na(year, total_score, p1, p2, p3, p4, p5, p6, total_medals) %>% 
  arrange(year) %>%
  mutate(
  ...
  ggplot(data, aes(
    ...
    geom_text(
      data = data %>% filter(highlight != "Other" & year == last_year),
       aes(label = glue::glue("{str_wrap(if_else(country == 'United States of America', 'USA', country), width = 30)} ({rank})")),
      hjust = -0.2,
    ...
    scale_y_reverse() +
    scale_country() +
    scale_x_date(date_breaks = "15 years", labels = label_date_short()) + 
...
# 3. Most steady
most_steady <- trends %>%
  filter(!country %in% c("Union of Soviet Socialist Republics", "Russian Federation")) %>% 
  group_by(country) %>%
...
plot_annotation(
  caption = "Tidytuesday week 39 | Graphic by Aman Bhargava | aman.bh",
