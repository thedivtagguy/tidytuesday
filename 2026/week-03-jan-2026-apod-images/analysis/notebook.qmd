...
Use a structured output schema to extract named celestial bodies from APOD descriptions.
...
Query Wikidata SPARQL to get RA/Dec coordinates for each object.
...
We'll filter out duplicates, misclassified objects, and keep only frequently mentioned items.
...
grid_x_pos <- map_dbl(
  ra_breaks_hours * 15,
  ~ {
    r <- ifelse(.x > 180, .x - 360, .x)
    (0.5 - (r / 360)) * info$width
  }
)
grid_x_minor <- map_dbl(
  ra_minor_hours * 15,
  ~ {
    r <- ifelse(.x > 180, .x - 360, .x)
    (0.5 - (r / 360)) * info$width
